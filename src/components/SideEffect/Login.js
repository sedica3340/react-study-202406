...
const Login = ({ onLogin }) => {
    ...
    // email reducer로 이메일 상대관리하기
    /**
     * param1: 위에서 만든 리듀서 함수
     * param2: 상태값의 초기값
     * return: reducer를 관리하는 배열
     *      [0]: 이메일 관련 상태값
     *      [1]: 상태를 변경하는 함수
     */
    ...
    // console.log("변경 후 이메일상태:", emailState.isValid);
    ...
    const emailChangeHandler = (e) => {
        ...
        // param1: reducer함수의 action에 전달할 내용
        dispatchEmail({
            type: "USER_INPUT",
            val: e.target.value,
        });
    ...
    const submitHandler = (e) => {
        ...
        onLogin(emailState.isValid, passwordState.isValid);
    };
    ...
    useEffect(() => {
        ...
        // cleanup 함수는 컴포넌트가 업데이트 되거나 사라지기 전에 실행
        return () => {
            clearTimeout(timer);
        };
